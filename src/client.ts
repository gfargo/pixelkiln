...
export class PixelLabClient {
   ...
   * default"). Exercised live at four sizes, all returning `last_response.
   ...
   * (1600px²) and 128x128 (16384px²), both billed 20 though the tiering
   * predicts 25 for that whole band (wrong, see estimate() in
   * pixellab.ts), and 512x512 (billed 40, the tiering ceiling). The real
   * 20->40 breakpoint is bisected to somewhere in (16384px², 262144px²].
   * `editImagesV2` below returns the same fields under `images`,
   * plural and array-wrapped, not `image` — do not assume the two endpoints
   * share one response shape. See docs/ENDPOINTS.md and docs/REVISIONS.md
   * for the full shape and cost picture, including what is still
   * unconfirmed.
   *
