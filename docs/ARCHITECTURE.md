...
- `history[]`: the generations this entry replaced, newest first — each one's
  object and job ids, durable sources, output hashes, prompt, cost, and when
  it was retired — capped by `PIXELKILN_HISTORY` or the manifest's `history`.
  A restore swaps a kept generation back in and moves the current one onto
  the list; the content cache keeps every kept generation's bytes through a
  prune.
