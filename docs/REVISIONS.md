...
`correct-pixelart`, `animate`, `animate-pixminimax`, and `animate-skeleton`,
not `outpaint` (its API has no canvas-expansion endpoint). ComfyUI covers
every mode generically except `animate`/`animate-pixminimax`/
`animate-skeleton`, which need an ordered frame set its revision path cannot
produce. Retro Diffusion and Scenario reject revision work during offline
resolution instead of silently starting a fresh text-to-image job.
...
`frames`, `fps`, the last frame's hash, `direction`, and `enhancePrompt`, or —
for `animate-skeleton` — the keypoints file's hash, `skeletonTemplate`, and
`description` (`direction` is recorded the same way; `frames` comes from the
keypoints file itself, same field, same staleness rule as the others).
