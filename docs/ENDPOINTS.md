...
- the character family beyond what the `character` generator uses: lip-sync,
  skeleton animation, and `source_image_id` on Pro Flash. The four creation
  engines, states, `animate-character`, portraits, and outfit transfer are
  covered in full; see the September 2026 measurements below.
